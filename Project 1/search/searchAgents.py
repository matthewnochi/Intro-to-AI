...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        if curr in self.corners and curr not in corners:
            corners.append(curr)
...
def cornersHeuristic(state, problem):
    ...
    while unvisited:
        distances = [(abs(curr[0] - corner[0]) + abs(curr[1] - corner[1]), corner) for corner in unvisited]

        min_distance, nearest_corner = min(distances)
        total_cost += min_distance
        next_position = nearest_corner
