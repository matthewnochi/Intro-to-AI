...
def enhancedPacmanFeatures(state, action):
    ...
    if foodList:
        ...
        for food in foodList:
            closest = min(util.manhattanDistance(newPos, food), closest)
        features["closest food"] = closest
        features["food count"] = len(foodList)

    features["nearPowerCapsule"] = 1 if any(util.manhattanDistance(newPos, cap) <= 2 for cap in capsules) else 0
    
    for i, ghostState in enumerate(newGhostStates):
        ghostPos = ghostState.getPosition() 
        features[f"ghost_{i}_distance"] = util.manhattanDistance(newPos, ghostPos)
